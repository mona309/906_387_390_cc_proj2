...
app.post("/append-entries", (req, res) => {
    ...
    // Valid leader contact — always reset election timer so we don't start spurious elections
    if (term > currentTerm) {
        currentTerm = term;
        votedFor = null;
    }
...
app.post("/append", async (req, res) => {
    ...
    let successCount = 1;
    const appendPromises = peers.map(async (peer) => {
        try {
    ...
    return res.json({ success: true, leaderId: NODE_ID, commitIndex });
});
